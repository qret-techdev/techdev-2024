...
def main():
  ...
  pidx = PID(0.047, 0.0010, 0.052, setpoint=0) #has been tuned a little bit
  ...
  window = make_window(sys_state, loc_x_y_filt, speed, accel)
  ...
  while 1:
    ...
    sys_state = update_window(window, frame, sys_state, loc_x_y_filt, speed, accel)
    ...
    if(sys_state==0): #keyboard control when in manual mode
      ...
      if(trip_init_guess==0):
        speed = [0, speedy_init_guess]
        trip_init_guess += 1

      #getting motor accelerations
      accel[0] = -pidx(loc_x_y_filt[0])
      accel[1] = -pidy(loc_x_y_filt[1])
      #updating speeds
      delta_t = time.time()-prev_time
